...
- Telegram flow: same Custom JWT infrastructure, verifier `blockster-telegram`. **Two paths**: desktop uses popup-based `Telegram.Login.auth({bot_id, …})` → `POST /api/auth/telegram/verify`. **Mobile uses redirect-based** `oauth.telegram.org/auth?bot_id=…&return_to=/api/auth/telegram/callback` (popup-mode hangs on iOS Safari + many in-app browsers because postMessage can't reach the parent). Mobile redirect: server stashes the Custom JWT in `:pending_telegram_jwt` session, bounces to `/?telegram_login=1`, JS hook on mount fetches the JWT via `GET /api/auth/telegram/pending_jwt` (one-shot, server clears) and runs `_startJwtLogin`. BotFather `/setdomain blockster.com` is required for both paths.
...
- **Signing pattern for Web3Auth**: derive Keypair, sign locally, `secretKey.fill(0)` in `finally`. Modal: `provider.request({method: "solana_privateKey"})` (fast — ws-embed has a local cache). SFA (mobile): `customauth.getTorusKey(...)` (~1s per call due to Torus DKG round-trip). Short-lived in-memory cache OK if latency hurts UX — see `docs/web3auth_sfa_migration.md` §3.7. Never write keys to persistent storage. See `assets/js/hooks/web3auth_hook.js` + `web3auth_sfa_hook.js`.
