...
### Phase B2: Deploy BUXBankroll — COMPLETE (Feb 18, 2026)
- **Deployed to Rogue Chain Mainnet** (Chain ID: 560013)
- **Proxy (ERC1967)**: `0xED7B00Ab2aDE39AC06d4518d16B465C514ba8630`
- **Implementation**: `0xb66db6C2815A4DF6Fe3915bd8323B2e7D54A5830`
- **Owner/Deployer**: `0x4BDC5602f2A3E04c6e3a9321A7AC5000e0A623e0`
- **Deploy tx**: `0xb6c4fd6c5abde7182b5e2f6980d404cff4afe8ee4b69cdfb4693d72e6ba5d078` (impl), `0x10ba32cb37f95c3bf76060a0d7fcbd922bfd87a29b26fd957cd7df0acc24741d` (proxy)
- **Post-deploy config**:
  - referralAdmin: `0xbD6feD8fEeec6f405657d0cA4A004f89F81B04ad` (same wallet as BuxBoosterGame BB admin — tx `0x9c2d01b83a271214b6fea1c7534b5279ec4f1fe3d1a409da4b1c1f67597d36fd`)
  - referralBasisPoints: 20 (0.2%) — set during initialize()
  - maximumBetSizeDivisor: 1000 — set during initialize()
  - LP price: 1e18 (1:1) — set during initialize()
  - plinkoGame: `0x7E12c7077556B142F8Fb695F70aAe0359a8be10C` — set in Phase P3 (Feb 19, 2026)
  - House deposit: NOT YET DONE — amount TBD
- **Verification**: 19/19 assertions passed
- **Scripts created**:
  - `scripts/deploy-bux-bankroll.js` — direct ethers.js deployment (bypasses Hardhat provider due to Rogue Chain RPC 500s on nginx)
  - `scripts/verify-bux-bankroll.js` — 19-assertion state verification
  - `scripts/setup-bux-bankroll.js` — referral admin, basis points, optional house deposit (idempotent)
- **Deployment lesson**: Hardhat's HTTP provider fails with 500s on `eth_sendRawTransaction` for large contracts on Rogue Chain. Use direct `ethers.JsonRpcProvider` + `ethers.ContractFactory` with explicit gasLimit. ERC1967Proxy artifact from `@openzeppelin/upgrades-core/artifacts/`.
- **Referral architecture decision**: BUXBankroll reuses the BuxBoosterGame referral admin wallet (`REFERRAL_ADMIN_BB_PRIVATE_KEY` / `txQueues.referralBB`). In the new architecture, referrals are managed at the bankroll level — BUXBankroll.playerReferrers is the canonical mapping for all BUX games. No new wallet or queue needed in Phase B3.

### Phase B3: BUX Minter Routes — COMPLETE (Feb 18, 2026)
- **File modified**: `bux-minter/index.js` (single-file Node.js minter service)
- **BUXBankroll contract setup**:
  - Added `BUX_BANKROLL_ADDRESS = "0xED7B00Ab2aDE39AC06d4518d16B465C514ba8630"`
  - Added `BUX_BANKROLL_ABI` with 9 functions: `getHouseInfo`, `getLPPrice`, `balanceOf`, `totalSupply`, `getPlinkoAccounting`, `getPlinkoPlayerStats`, `getMaxBet`, `setPlayerReferrer`, `playerReferrers`
  - Created read-only `buxBankrollContract` (via `provider`)
  - Created write `buxBankrollWriteContract` (via `referralAdminBBWallet` — same wallet as BuxBoosterGame referrals)
- **New read-only endpoints** (all behind `authenticate` middleware):
  - `GET /bux-bankroll/house-info` — 6 fields (totalBalance, liability, unsettledBets, netBalance, poolTokenSupply, poolTokenPrice)
  - `GET /bux-bankroll/lp-price` — LP-BUX price as string
  - `GET /bux-bankroll/lp-balance/:address` — user's LP-BUX balance as string
  - `GET /bux-bankroll/player-stats/:address` — 9 fields including betsPerConfig[9]/pnlPerConfig[9] arrays
  - `GET /bux-bankroll/accounting` — 11 fields including signed int256 values (totalHouseProfit, houseEdge)
  - `TODO`: `GET /bux-bankroll/max-bet/:configIndex` — skipped, requires PlinkoGame contract (Phase P3)
- **Updated `/set-player-referrer`**:
  - Now sets referrer on THREE contracts: BuxBoosterGame, ROGUEBankroll, BUXBankroll
  - Pre-checks all three in parallel for existing referrer
  - BuxBoosterGame + BUXBankroll execute SEQUENTIALLY on `referralBB` queue (same wallet)
  - ROGUEBankroll executes in PARALLEL on `referralRB` queue (different wallet)
  - Updated "all already set" 409 check and 503 guard to include `buxBankrollWriteContract`
  - Response `results` object now includes `buxBankroll: { success, txHash, error }`
- **Updated `/queue-status`**: Added `contracts` object with all 4 contract addresses
- **Updated startup log**: Logs BUXBankroll address and updated referral system message
- **Syntax check**: `node -c bux-minter/index.js` passes clean
- **Deployed to Fly.io**: `flyctl deploy --app bux-minter` (Feb 19, 2026) — includes Plinko endpoints (Phase P4)
- **NOT committed** — awaiting user instructions

### Phase B4: Backend (LPBuxPriceTracker, Mnesia, BuxMinter) — COMPLETE (Feb 18, 2026)
- **Worktree**: `../blockster-v2-bankroll` on branch `feat/bux-bankroll`
- **Files modified**:
  - `lib/blockster_v2/bux_minter.ex` — Added 5 new public functions for BUX Bankroll API calls:
    - `bux_bankroll_house_info/0` → GET `/bux-bankroll/house-info` (returns full map with 6 fields)
    - `bux_bankroll_lp_price/0` → GET `/bux-bankroll/lp-price` (returns `response["price"]` string)
    - `bux_bankroll_lp_balance/1` → GET `/bux-bankroll/lp-balance/:address` (returns `response["balance"]` string)
    - `bux_bankroll_player_stats/1` → GET `/bux-bankroll/player-stats/:address` (returns full map with 9 fields)
    - `bux_bankroll_accounting/0` → GET `/bux-bankroll/accounting` (returns full map with 11 fields)
  - `lib/blockster_v2/mnesia_initializer.ex` — Added `:lp_bux_candles` table to `@tables` list (ordered_set, 5 attrs: timestamp/open/high/low/close, no indexes)
  - `lib/blockster_v2/application.ex` — Added `{BlocksterV2.LPBuxPriceTracker, []}` to `genserver_children` after PriceTracker
  - `lib/blockster_v2_web/router.ex` — Added `live "/bankroll", BankrollLive, :index` in `:default` live_session after `/play`
- **Files created**:
  - `lib/blockster_v2/lp_bux_price_tracker.ex` — GlobalSingleton GenServer:
    - Polls LP-BUX price every 60 seconds via `BuxMinter.bux_bankroll_lp_price/0`
    - Stores 5-minute OHLC candles in Mnesia `:lp_bux_candles` table
    - Broadcasts `{:lp_bux_price_updated, %{price: float, candle: map}}` on PubSub topic `"lp_bux_price"`
    - Follows PriceTracker pattern exactly: GlobalSingleton start_link, :registered message, :wait_for_mnesia loop, global ownership checks
    - Public API: `get_current_price/0`, `get_candles/2`, `get_stats/0`, `refresh_price/0`
    - `get_candles/2` reads base 5-min candles from Mnesia and aggregates to target timeframe (e.g., 3600s for hourly)
    - `parse_price/1` handles both wei strings (integer / 1e18) and float strings
  - `test/blockster_v2/lp_bux_price_tracker_test.exs` — 15 tests:
    - get_candles/2 (7): empty list, raw 5-min, hourly aggregation, correct high/low, correct open/close, sorted output, count limiting
    - get_stats/0 (3): all timeframe keys present, nil when no data, correct high/low values
    - Mnesia integration (3): ordered_set ordering, dirty_select range filter, overwrite behavior
- **Compile check**: `mix compile` passes. Only new warning is expected `BankrollLive` module not yet defined (Phase B5)
- **Skipped** (as planned):
  - `bux_bankroll_max_bet/1` — minter endpoint doesn't exist yet (Phase P3)
  - `:plinko_games` Mnesia table — Phase P5
  - `PlinkoSettler` GenServer — Phase P5
  - `/plinko` route — Phase P5
  - `BankrollLive` module — Phase B5
- **Tests**: 13/13 passing (`mix test test/blockster_v2/lp_bux_price_tracker_test.exs`)
  - Note: Only 13 of the original 15 test cases are included (2 were duplicates removed during cleanup)
- **NOT committed** — awaiting user instructions

### Phase B5: BankrollLive + Chart/Onchain hooks — COMPLETE (Feb 18, 2026)
- **Worktree**: `../blockster-v2-bankroll` on branch `feat/bux-bankroll`
- **Files created**:
  - `lib/blockster_v2_web/live/bankroll_live.ex` — Full LiveView module (813 lines, inline HEEx template)
    - Mount: PubSub subscriptions (`lp_bux_price`, `token_prices`, `bux_balance:{user_id}`), async pool info/candles/LP balance fetch
    - Render: pool stats bar (loading → data), candlestick chart with 5 timeframe tabs, deposit/withdraw panel (auth), login prompt (unauth), "Your Position" section, "How It Works"
    - Event handlers: select_timeframe, set_action_tab, update_deposit/withdraw_amount, set_max_deposit/withdraw, deposit_bux, withdraw_bux, deposit/withdraw_confirmed/failed
    - Async handlers: fetch_pool_info, fetch_candles, fetch_lp_balance (all with success + crash clauses)
    - PubSub handlers: lp_bux_price_updated, bux_balance_updated, token_balances_updated, token_prices_updated
    - Helpers: format_balance, format_lp_price, format_withdrawal_price, parse_amount, preview_lp_tokens, preview_bux_out, extract_price_range, parse_pool_value, parse_pool_float, parse_lp_balance, get_rogue_price, timeframe_order
    - Deposit preview uses `effectiveBalance = totalBalance - unsettledBets` for fair LP pricing
    - Withdrawal preview uses `netBalance = totalBalance - liability` for safety
    - `get_rogue_price/0` has rescue/catch for missing Mnesia tables (PriceTracker.get_price crashes if `:token_prices` table doesn't exist in test env)
  - `assets/js/lp_bux_chart.js` — LPBuxChart hook (91 lines)
    - Uses `createChart` and `CandlestickSeries` from lightweight-charts (installed via npm)
    - Handles `set_candles` (full data load) and `update_candle` (real-time) events
    - ResizeObserver for responsive sizing
    - Dark theme matching zinc-900 background, up candles: #CAFC00, down candles: #ef4444
  - `assets/js/bankroll_onchain.js` — BankrollOnchain hook (121 lines)
    - Thirdweb integration: checks allowance, approves if needed (infinite approval), calls depositBUX/withdrawBUX
    - Waits for receipt before pushing confirmation events to LiveView
    - Error parser maps contract errors to human-readable messages (InsufficientBalance, InsufficientLiquidity, ZeroAmount, etc.)
  - `test/blockster_v2_web/live/bankroll_live_test.exs` — 24 tests across 7 describe blocks, all passing
    - Mount (unauth): page title, pool stats loading → loaded, login prompt, no "Your Position", How It Works
    - Mount (auth): deposit/withdraw tabs, deposit form default, user BUX balance
    - Timeframe selection: clicking tab updates selected, all 5 buttons rendered
    - Deposit flow: LP preview, zero amount validation (server-side via render_hook), confirmed/failed events, set_max_deposit
    - Withdraw flow: tab switch, zero amount validation, confirmed/failed events
    - PubSub: lp_bux_price_updated updates displayed price
    - Chart: container present with hook, phx-update="ignore"
    - Tab switching: default deposit, switch to withdraw, error clears on tab switch
- **Files modified**:
  - `assets/js/app.js` — Imported and registered LPBuxChart + BankrollOnchain hooks
- **npm package installed**: `lightweight-charts` (TradingView charting library)
- **Compile check**: `mix compile` passes clean
- **Test notes**:
  - Pool stats load async via `start_async` — initial render shows "Loading pool data..." spinner, tests wait 300ms for async completion before asserting "Total BUX"
  - Deposit/withdraw buttons are HTML `disabled` when amount is 0/"" — tests use `render_hook` to send events directly for server-side validation testing
  - BuxMinter calls return `{:error, :not_configured}` in test env — handled gracefully by async error clauses
  - PriceTracker.get_price crashes if `:token_prices` Mnesia table missing — handled by rescue/catch in `get_rogue_price/0`
- **NOT committed** — awaiting user instructions
...
| B4 | `lp_bux_price_tracker_test.exs` | 13 |
| B5 | `bankroll_live_test.exs` | 24 |
| **Total** | | **~107 tests + 15 deploy assertions** |
