...
- ✅ **Referral UI parked (2026-04-27)** — every user-visible referral surface stripped: `/notifications/referrals` route + LV deleted, "Refer" tab + earnings card on `/member/:slug` deleted, "Referral Dashboard" link card on `/notifications` deleted, announcement-banner referral messages neutered, `/docs/pools` Referrals section + sidebar entry removed, `/docs/coin-flip` referral subsection removed. **No new mainnet user gets BUX or SOL from any referral flow on day 1.** Backend dead-code preserved for future revival: `BlocksterV2.Referrals` context module intact, Mnesia tables (`:referrals`, `:referral_earnings`, `:referral_stats`) still defined in `mnesia_initializer.ex`, on-chain Anchor `set_referrer` instruction + `referral_bps` / `tier2_referral_bps` fields untouched on the deployed bankroll program. To revive: re-add the UI surface, hook `Referrals.process_signup_referral/2` into `Accounts.create_user_from_web3auth/2`, optionally build the settler `/set-player-referrer` route + pass referrer accounts in `settle_bet` tx builder. See `docs/solana_build_history.md` 2026-04-27 entry for the full removal log + revival path.
