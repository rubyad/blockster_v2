...
#### Phase 2b ✅ COMPLETE (2026-04-14)

- [x] `assets/css/widgets.css` — scoped `.bw-widget` tokens, Satoshi + JetBrains Mono font-family hooks, `bw-card/bw-shell/bw-pulse-*/bw-flash-*` utilities, `.bw-scroll` scrollbar. Every selector is descendant-scoped (`.bw-widget .bw-card`) so RogueTrader/FateSwap dark tokens never leak into Blockster's white design system.
- [x] Satoshi (400/500/700/900) + JetBrains Mono (400/500/600/700) `<link>` tags in `root.html.heex`, with `<link rel="preconnect">` for `api.fontshare.com`. `display=swap` accepts a brief reflow rather than blocking paint.
- [x] `@import "./widgets.css"` in `assets/css/app.css` (after Tailwind `@source` directives).
- [x] `lib/blockster_v2_web/live/widget_events.ex` — `use BlocksterV2Web.WidgetEvents` macro. `mount_widgets/2` gated on `Phoenix.LiveView.connected?/1`, subscribes to 3 data topics + per-banner `widgets:selection:*`, pre-subscribes to chart topics for cached RT selections, seeds initial assigns from local Mnesia (no cross-node call), increments impressions once per banner. `handle_info` for all 4 message shapes — `{:selection_changed, _, nil}` is a deliberate no-op so `WidgetSelector` nil-for-unknown-mode doesn't push stale events. `handle_event("widget_click", …)` normalises DOM subjects: tuples come back as `%{"bot_id" => _, "tf" => _}` maps (atom keys stringify on the wire); binary order_ids and `"rt"`/`"fs"` strings pass through; `banner_id` is parsed string→integer with short-circuit on unparseable values.
- [x] `lib/blockster_v2_web/components/widget_components.ex` — `widget_or_ad/1` function component. `widget_type: nil` → existing `<BlocksterV2Web.DesignSystem.ad_banner banner={...} />`. Known widget_type → `ArgumentError` with `"widget component not yet implemented (Phase 3+): #{type}"`. Unknown widget_type → `ArgumentError` with `"unknown widget_type: ..."` so mis-typed admin configs surface loudly.
- [x] Tests: 28 new (2775 total / 117 failures; zero new vs Phase 2a baseline). Macro covered via `test/support/widget_events_test_host.ex` + `live_isolated/3` — PubSub subscription verified by broadcasting on each topic and checking `assert_push_event`, impression counter verified per banner, all 4 `handle_info` shapes, all 4 `widget_click` subject shapes. Dispatcher covered by `render_component/2` — nil → `ad_banner`, every valid widget_type raises Phase-3+ message, unknown widget_type raises unknown-type message.

**Phase 2b deviations from plan (now load-bearing)**

1. **`refute_push_event` with an interpolated event name doesn't compile** — the macro pastes the event AST into a `receive do` pattern and `^banner_event` is not a legal top-level expression outside a match context. For "no push" assertions with a dynamic banner_id, verify side effects indirectly: check the impressions counter stayed at 0 (for nil-widget-type skip path) or assert a follow-up `{:fs_trades, []}` round-trips (for nil-subject no-op path, proving the LV didn't crash).
2. **`assert_push_event` DOES accept a pinned variable for the event name** (`^event_name`) — bind the interpolated string to a variable before calling. Passing a bare variable silently succeeds by binding (matches anything), which is a test-always-green footgun.
3. **`render_hook(view, "widget_click", …)` returns `{:error, {:redirect, %{to: url, status: 302}}}`** for `redirect(socket, external: url)`. The LiveViewTest boundary normalises `:external` to `:to` — tests match on `:to`, production code still uses `external:` semantics (the HTTP response carries the external URL).
4. **Impression increment lives inside the `connected?/1` branch** of `mount_widgets/2`, not at the top. LiveView's double-mount calls disconnected → connected; counting impressions on both would inflate by 2×. The disconnected render is also not a real user view, so counting it is wrong anyway.
