...
| 4 | Cart | 🔧 Built, awaiting commit | — | Full template rewrite. Per-item BUX redemption + sticky order summary + suggested products + empty state. `max_bux_for_item` bug fix (0=uncapped). Hub preload added. 17 new tests. 0 new failures vs baseline. |
| 4–6 | Checkout, Wallet, Category, Tag, Notifications, Onboarding | ⬜ Not started | — | Checkout (#14) is next. |
