...
| 6 | Onboarding | ✅ Done | `ec5ed8f` | Full template rewrite. DS color tokens + typography + card patterns + segmented progress bar. 8 step components restyled. All 14 handlers preserved. Route stays on `:onboarding` (intentionally no DS header/footer). 9 new tests (18 total). 0 new failures vs baseline. |
