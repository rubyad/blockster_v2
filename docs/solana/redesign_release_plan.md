...
| 4 | Shop index | 🔧 Built, awaiting commit | — | Full template rewrite. Full-bleed hero banner, sidebar filter with per-filter counts, 3-col product grid, DS header (active="shop"), 17 new tests. 0 new failures vs baseline. |
| 4–6 | Product detail, Cart, Checkout, Wallet, Category, Tag, Notifications, Onboarding | ⬜ Not started | — | Product detail (#12) is next. |
