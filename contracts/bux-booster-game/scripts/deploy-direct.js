/**
 * Direct deployment script using ethers.js without Hardhat's provider
 * This bypasses Hardhat's HTTP handling which may be causing issues
 */
...
async function main() {
  // Setup provider and wallet using standard JsonRpcProvider
  ...
  const artifactPath = path.join(__dirname, "../artifacts/contracts/BuxBoosterGame.sol/BuxBoosterGame.json");
  ...
  const implementation = await retryWithBackoff(async () => {
    ...
    // Deploy with explicit gas limit only
  ...
  // For simplicity, we'll call initialize directly on the implementation
  // In production, you'd deploy a proxy. For now, let's use the implementation directly.
  console.log("\nInitializing contract...");
  await retryWithBackoff(async () => {
    const tx = await implementation.initialize(settlerAddress, treasuryAddress, {
      gasLimit: 500000,
      gasPrice: 1000000000
    });
  ...
  const treasury = await implementation.treasury();
