    ...
    PausableUpgradeable
{
    ...
    function settleBet(
        ...
        // Emit settlement events (split into two helpers to avoid stack-too-deep)
        _emitBetSettled(commitmentHash, bet.player, bet.amount, payout, multiplierBps, landingPosition, serverSeed);
        _emitBetDetails(commitmentHash, bet.token, bet.amount, bet.configIndex, bet.nonce, bet.timestamp, landingPosition, path);
    }
    ...
    /**
     * @dev Emit PlinkoBetSettled event. Split from _emitBetDetails to avoid stack-too-deep.
     */
    function _emitBetSettled(
        bytes32 commitmentHash,
        address player,
        uint256 amount,
        uint256 payout,
    ...
    /**
     * @dev Emit PlinkoBallPath and PlinkoBetDetails events.
     */
    function _emitBetDetails(
        bytes32 commitmentHash,
        address token,
        uint256 amount,
        uint8 configIndex,
