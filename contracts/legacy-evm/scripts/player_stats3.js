...
const DIFFICULTIES = [
  { level: -4, name: "Win One 5-flip", mult: "1.02x" },
  { level: -3, name: "Win One 4-flip", mult: "1.05x" },
  { level: -2, name: "Win One 3-flip", mult: "1.13x" },
  { level: -1, name: "Win One 2-flip", mult: "1.32x" },
  { level: 0, name: "Single Flip", mult: "1.98x" },
  { level: 1, name: "Win All 2-flip", mult: "3.96x" },
  { level: 2, name: "Win All 3-flip", mult: "7.92x" },
  { level: 3, name: "Win All 4-flip", mult: "15.84x" },
  { level: 4, name: "Win All 5-flip", mult: "31.68x" }
];
...
async function main() {
  ...
  // BUX stats from BuxBoosterGame contract
  console.log("--- BUX Stats (from BuxBoosterGame contract) ---");
  try {
    const buxBooster = new ethers.Contract(BUX_BOOSTER_GAME_ADDRESS, BUX_ABI, provider);
    const stats = await buxBooster.getPlayerStats(PLAYER_ADDRESS);
    ...
    console.log("");
    console.log("Per-Difficulty Breakdown (BUX):");
    for (let i = 0; i < 9; i++) {
  ...
  // ROGUE stats from ROGUEBankroll contract
  console.log("--- ROGUE Stats (from ROGUEBankroll contract) ---");
  try {
