defmodule BlocksterV2Web.CampaignAdminLive.Edit do
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    # Load selected users if custom audience
    selected_users = load_selected_users(campaign)
    ...
    {:ok,
     ...
     |> assign(:steps, @steps)
     |> assign(:hubs, hubs)
     |> assign(:form_data, campaign_to_form_data(campaign))
    ...
    <div class="min-h-screen bg-[#F5F6FB]">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <div class="flex items-center gap-4 mb-8">
          <.link navigate={~p"/admin/notifications/campaigns/#{@campaign.id}"} class="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm border border-gray-100 hover:bg-gray-50 cursor-pointer">
          ...
          <div>
            <h1 class="text-2xl font-haas_medium_65 text-[#141414]">Edit Campaign</h1>
            <p class="text-sm text-gray-500 font-haas_roman_55"><%= @campaign.name %> · Step <%= step_index(@step) + 1 %> of <%= length(@steps) %></p>
          </div>
        </div>
  ...
  defp step_schedule(assigns) do
    ...
    <div class="space-y-5">
      ...
      <div>
        <label class="block text-sm font-haas_medium_65 text-gray-700 mb-1">Scheduled Date & Time (UTC)</label>
        ...
        <p class="text-xs text-gray-500 font-haas_roman_55 mt-1">Leave empty for manual send</p>
      </div>
    </div>
    """
  end
  ...
  defp step_review(assigns) do
    ~H"""
    <div class="space-y-6">
      <h2 class="text-lg font-haas_medium_65 text-[#141414] mb-4">Review Changes</h2>
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="p-4 bg-[#F5F6FB] rounded-xl">
          <div class="text-xs text-gray-500 font-haas_roman_55 mb-1">Schedule</div>
          <div class="text-sm font-haas_medium_65 text-[#141414]"><%= if @form_data["scheduled_at"] && @form_data["scheduled_at"] != "", do: @form_data["scheduled_at"], else: "Not scheduled" %></div>
        </div>
      </div>
  ...
  defp campaign_to_form_data(%Campaign{} = c) do
    criteria = c.target_criteria || %{}

    %{
      "name" => c.name || "",
      "type" => c.type || "email_blast",
      "subject" => c.subject || "",
      "title" => c.title || "",
      "body" => c.body || "",
      "plain_text_body" => c.plain_text_body || "",
      "image_url" => c.image_url || "",
      "action_url" => c.action_url || "",
      "action_label" => c.action_label || "Learn More",
      "target_audience" => c.target_audience || "all",
      "target_hub_id" => if(c.target_hub_id, do: to_string(c.target_hub_id)),
      "send_email" => c.send_email,
      "send_in_app" => c.send_in_app,
      "send_sms" => c.send_sms,
      "scheduled_at" => format_scheduled_at(c.scheduled_at),
      "send_now" => is_nil(c.scheduled_at),
      "balance_operator" => criteria["operator"] || "above",
      "balance_threshold" => criteria["threshold"] || "",
      "wallet_provider" => criteria["provider"] || "metamask",
      "multiplier_operator" => criteria["operator"] || "above",
      "multiplier_threshold" => criteria["threshold"] || ""
    }
  end

  defp format_scheduled_at(nil), do: ""
  defp format_scheduled_at(dt) do
    Calendar.strftime(dt, "%Y-%m-%dT%H:%M")
  end

  defp load_selected_users(%Campaign{target_audience: "custom", target_criteria: %{"user_ids" => ids}})
  ...
  defp parse_scheduled_at(dt_string) do
    padded = if String.length(dt_string) == 16, do: dt_string <> ":00", else: dt_string
