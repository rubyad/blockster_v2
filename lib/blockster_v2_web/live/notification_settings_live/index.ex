defmodule BlocksterV2Web.NotificationSettingsLive.Index do
  ...
  defp settings_section(assigns) do
    ~H"""
    <div class="mb-5 bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden">
      <div class="p-5">
        <div class="flex items-center gap-2 mb-0.5">
          <div class="w-1 h-4 bg-[#CAFC00] rounded-full"></div>
          <h2 class="text-base font-bold text-[#141414]">{@title}</h2>
        </div>
        <%= if @subtitle do %>
          <p class="text-xs text-neutral-400 ml-3 mb-4">{@subtitle}</p>
        <% end %>
  ...
  defp toggle(assigns) do
    ...
    <div class="flex items-center justify-between py-2.5 group/toggle">
      <div>
        <p class="text-sm text-[#141414]">{@label}</p>
        <%= if @description do %>
          <p class="text-[11px] text-neutral-400">{@description}</p>
        <% end %>
      </div>
      <button
        phx-click="toggle_preference"
        phx-value-field={@field}
        class={"relative inline-flex h-7 w-12 items-center rounded-full transition-colors duration-200 cursor-pointer " <>
          if(@checked, do: "bg-[#0a0a0a]", else: "bg-neutral-200 group-hover/toggle:bg-neutral-300")}
        role="switch"
        aria-checked={to_string(@checked)}
      >
        <span class={"inline-block h-5 w-5 transform rounded-full bg-white shadow-[0_1px_3px_rgba(0,0,0,0.1)] transition-transform duration-200 " <>
          if(@checked, do: "translate-x-6", else: "translate-x-0.5")}></span>
      </button>
    </div>
    """
  end
  ...
  defp hub_toggle(assigns) do
    ...
    <button
      ...
      phx-value-field={@field}
      class={"flex items-center gap-2 px-3 py-2 rounded-xl text-xs transition-all cursor-pointer " <>
        if(@checked, do: "bg-white text-[#141414] shadow-[0_1px_2px_rgba(0,0,0,0.04)]", else: "bg-transparent text-neutral-400 hover:bg-white/50")}
    >
      <div class={"w-3.5 h-3.5 rounded-md border-2 transition-colors " <> if(@checked, do: "border-[#0a0a0a] bg-[#0a0a0a]", else: "border-neutral-300")}></div>
      {@label}
    </button>
    """
  end
  ...
  announcement_banner={assigns[:announcement_banner]}
    ...
    <main class="max-w-[640px] mx-auto px-6">
      ...
      <section class="pt-12 pb-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <h1 class="text-2xl font-bold text-[#141414]">Notification Settings</h1>
          </div>
          <%= if @saved do %>
            <span class="flex items-center gap-1 text-xs font-medium text-white bg-[#0a0a0a] px-2.5 py-1 rounded-lg">
      ...
      <%= if @preferences do %>
        <%!-- Email Toggle --%>
        <.settings_section title="Email Notifications" subtitle="Receive emails for articles, rewards, and updates">
          <.toggle
        ...
        <%!-- In-App Toggle --%>
        <.settings_section title="In-App Notifications" subtitle="See notifications in the bell icon while browsing">
        ...
        <%!-- Telegram --%>
        <.settings_section title="Telegram" subtitle="Get new posts delivered to your Telegram">
          <div class="py-2 space-y-4">
              ...
              <div class="flex-1">
                <%= if @telegram_connected do %>
                  <div class="flex items-center gap-2 mb-1">
                    <p class="text-sm font-bold text-[#141414]">Telegram Connected</p>
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 text-green-600 text-[10px] font-bold rounded-full">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-3 h-3">
                        <path fill-rule="evenodd" d="M19.916 4.626a.75.75 0 0 1 .208 1.04l-9 13.5a.75.75 0 0 1-1.154.114l-6-6a.75.75 0 0 1 1.06-1.06l5.353 5.353 8.493-12.74a.75.75 0 0 1 1.04-.207Z" clip-rule="evenodd" />
                      </svg>
                      Connected
                  ...
                  <p class="text-xs text-neutral-400 mb-2">
                    Linked as <span class="text-neutral-600 font-bold">@{@telegram_username || "unknown"}</span>
                  </p>
                  <%= if @show_telegram_disconnect do %>
                    <div class="flex items-center gap-2">
                      <button
                        phx-click="confirm_telegram_disconnect"
                        class="px-3 py-1.5 bg-red-600 text-white text-xs font-bold rounded-lg hover:bg-red-700 transition-colors cursor-pointer"
                      >
                        Yes, disconnect
                      </button>
                      <button
                        phx-click="cancel_telegram_disconnect"
                        class="px-3 py-1.5 bg-neutral-100 text-neutral-600 text-xs rounded-lg hover:bg-neutral-200 transition-colors cursor-pointer"
                      >
                        Cancel
                      </button>
                    </div>
                  <% else %>
                    <button
                      ...
                      class="text-xs text-red-400 hover:text-red-600 font-bold cursor-pointer transition-colors"
                    >
                      Disconnect
                    </button>
                  <% end %>
                <% else %>
                  <p class="text-sm font-bold text-[#141414] mb-1">Connect your Telegram</p>
                  <p class="text-xs text-neutral-400 mb-3">
                    Link your Telegram account to Blockster and earn <span class="text-[#141414] font-bold">500 BUX</span>.
                  </p>
                  <%= if @telegram_bot_url do %>
                    <a
                      ...
                      class="inline-flex items-center gap-2 px-4 py-2.5 bg-[#2AABEE] text-white text-sm font-bold rounded-xl hover:bg-[#229ED9] transition-colors cursor-pointer"
                    >
                    ...
                    <p class="text-xs text-neutral-400 mt-2 flex items-center gap-1.5">
            ...
            <div class="pt-3 border-t border-neutral-100">
              <div class="flex items-center gap-2 mb-1">
                <p class="text-sm font-bold text-[#141414]">Blockster V2 Group</p>
              ...
              <%= if @telegram_group_joined do %>
                <p class="text-xs text-neutral-400 mb-3">You're a member of the Blockster Telegram group.</p>
              <% else %>
                <p class="text-xs text-neutral-400 mb-3">Join our Telegram group and earn <span class="text-[#141414] font-bold">500 BUX</span>.</p>
              <% end %>
              <a
                href="https://t.me/+7bIzOyrYBEc3OTdh"
                target="_blank"
                rel="noopener noreferrer"
                class="inline-flex items-center gap-2 px-4 py-2.5 bg-[#0a0a0a] text-white text-sm font-bold rounded-xl hover:bg-neutral-800 transition-colors cursor-pointer"
              >
        ...
        <%!-- Per-Hub Settings --%>
        <%= if @followed_hubs != [] do %>
          <.settings_section title="Hub Notifications" subtitle="Control notifications per hub you follow">
            <div class="space-y-3">
                  ...
                  <div class="flex items-center gap-3 mb-3">
                    ...
                    <% else %>
                      <div class="w-9 h-9 rounded-xl bg-[#CAFC00] flex items-center justify-center">
                        <span class="text-sm font-bold text-black">{String.first(entry.hub.name)}</span>
                      </div>
                    <% end %>
                    <p class="text-sm font-bold text-[#141414]">{entry.hub.name}</p>
                  </div>
        ...
        <%!-- Unsubscribe All --%>
        <div class="mt-6 mb-8 p-5 bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
          <%= if @show_unsubscribe_confirm do %>
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="text-sm font-bold text-red-600 mb-1">Are you sure?</p>
                <p class="text-xs text-neutral-400 mb-4">This will disable all email and SMS notifications. You can re-enable them later.</p>
                <div class="flex gap-2">
                  <button
                    phx-click="confirm_unsubscribe_all"
                    class="px-4 py-2 bg-red-600 text-white text-xs font-bold rounded-lg hover:bg-red-700 transition-colors cursor-pointer"
                  >
                    Yes, unsubscribe
                  </button>
                  <button
                    phx-click="cancel_unsubscribe"
                    class="px-4 py-2 bg-neutral-100 text-neutral-600 text-xs rounded-lg hover:bg-neutral-200 transition-colors cursor-pointer"
                  >
                    Cancel
                  </button>
                </div>
          ...
          <% else %>
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm text-neutral-500">Want to stop all notifications?</p>
              </div>
              <button
      ...
      <% else %>
        <div class="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p class="text-neutral-400">Please log in to manage your notification settings.</p>
        </div>
      <% end %>
