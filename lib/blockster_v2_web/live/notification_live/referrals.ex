defmodule BlocksterV2Web.NotificationLive.Referrals do
  ...
  def mount(_params, _session, socket) do
    ...
    if user do
      ...
      {:ok,
       ...
       |> assign(:earnings, [])
       |> assign(:config, %{referrer_signup_bux: 500, referee_signup_bux: 250, phone_verify_bux: 500})
  ...
  defp truncate_wallet(w) when byte_size(w) > 10 do
    "#{String.slice(w, 0..3)}...#{String.slice(w, -4..-1)}"
  end
  ...
  defp type_style(_), do: "bg-neutral-100 text-neutral-700"
  ...
  def render(assigns) do
    ...
    <main class="max-w-[720px] mx-auto px-6">
          ...
          <div>
            <div class="flex items-center gap-3 mb-3">
              <.link
                ...
                class="flex items-center justify-center w-8 h-8 rounded-lg bg-white border border-neutral-200/70 hover:border-neutral-300 hover:shadow-sm transition-all cursor-pointer"
              ...
              <BlocksterV2Web.DesignSystem.eyebrow>Referrals</BlocksterV2Web.DesignSystem.eyebrow>
            </div>
            <h1 class="font-bold tracking-[-0.022em] leading-[0.96] text-[36px] md:text-[48px] text-[#141414]">
              Referral Dashboard
            </h1>
            <p class="text-sm text-neutral-500 mt-2">Track your referrals and earnings</p>
          </div>
        </div>
      </section>
      ...
      <section class="pb-6">
        <%= if @referral_link do %>
          <div class="bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-5 sm:p-6">
            <h3 class="text-base font-bold text-[#141414] mb-2">Your Referral Link</h3>
            <p class="text-xs text-neutral-500 mb-4">
              You earn <span class="font-bold text-[#141414]">{@config.referrer_signup_bux} BUX</span> per signup.
              Your friend gets <span class="font-bold text-[#141414]">{@config.referee_signup_bux} BUX</span> too!
            </p>
            ...
            <div class="flex flex-col sm:flex-row gap-2 mb-4">
              <input
                ...
                readonly
                value={@referral_link}
                class="flex-1 px-4 py-2.5 bg-neutral-50 border border-neutral-200/70 rounded-xl text-xs font-mono text-neutral-600 min-w-0"
              />
              <button
                id="copy-referral-link"
                phx-hook="CopyToClipboard"
                data-copy-text={@referral_link}
                class="cursor-pointer px-5 py-2.5 bg-[#0a0a0a] text-white rounded-xl text-sm font-medium hover:bg-neutral-800 transition-colors flex items-center justify-center gap-2 flex-shrink-0"
              >
            ...
            <div class="flex items-center gap-2">
              <span class="text-xs text-neutral-400">Share via:</span>
              <a
                ...
                class="flex items-center justify-center w-8 h-8 rounded-lg bg-neutral-50 border border-neutral-200/70 hover:border-neutral-300 transition-all cursor-pointer"
                title="Share on X"
              >
                <svg class="w-3.5 h-3.5 text-neutral-700" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </a>
              <a
              ...
              >
                <svg class="w-4 h-4 text-neutral-700" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
              </a>
              <a
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4 text-neutral-700">
        ...
        <% else %>
          <div class="bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-6 text-center">
            <p class="text-sm text-neutral-500">
              <.link navigate={~p"/"} class="text-[#141414] font-bold hover:underline cursor-pointer">Connect your wallet</.link>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div class="bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4">
            <div class="flex items-center gap-2 mb-2">
              ...
              <span class="text-[10px] text-neutral-500 font-bold uppercase tracking-[0.16em]">Referrals</span>
            </div>
            <div class="text-2xl font-bold text-[#141414]">{@stats.total_referrals}</div>
          </div>
          <div class="bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4">
            <div class="flex items-center gap-2 mb-2">
              <div class="w-2.5 h-2.5 rounded-full bg-emerald-500"></div>
              <span class="text-[10px] text-neutral-500 font-bold uppercase tracking-[0.16em]">Verified</span>
            </div>
            <div class="text-2xl font-bold text-[#141414]">{@stats.verified_referrals}</div>
          </div>
          <div class="bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4">
            <div class="flex items-center gap-2 mb-2">
              <div class="w-2.5 h-2.5 rounded-full bg-amber-500"></div>
              <span class="text-[10px] text-neutral-500 font-bold uppercase tracking-[0.16em]">BUX Earned</span>
            </div>
            <div class="text-2xl font-bold text-[#141414]">{format_number(@stats.total_bux_earned)}</div>
          </div>
          <div class="bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4">
            <div class="flex items-center gap-2 mb-2">
              <div class="w-2.5 h-2.5 rounded-full bg-purple-500"></div>
              <span class="text-[10px] text-neutral-500 font-bold uppercase tracking-[0.16em]">ROGUE Earned</span>
            </div>
            <div class="text-2xl font-bold text-[#141414]">{format_number(@stats.total_rogue_earned)}</div>
          </div>
        </div>
      </section>
      ...
      <section class="pb-6">
        <div class="bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-5">
          <h3 class="text-sm font-bold text-[#141414] mb-3">How Referral Rewards Work</h3>
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div class="flex items-center gap-3 p-3 bg-neutral-50 rounded-xl">
              ...
              <div>
                <p class="text-xs font-bold text-[#141414]">Friend signs up</p>
                <p class="text-[10px] text-neutral-500">You get {@config.referrer_signup_bux} BUX, they get {@config.referee_signup_bux} BUX</p>
              </div>
            </div>
            <div class="flex items-center gap-3 p-3 bg-neutral-50 rounded-xl">
              <div class="w-8 h-8 bg-[#CAFC00] rounded-lg flex items-center justify-center flex-shrink-0">
              ...
              <div>
                <p class="text-xs font-bold text-[#141414]">Friend verifies phone</p>
                <p class="text-[10px] text-neutral-500">You get {@config.phone_verify_bux} BUX</p>
              </div>
            </div>
            <div class="flex items-center gap-3 p-3 bg-neutral-50 rounded-xl">
              <div class="w-8 h-8 bg-[#CAFC00] rounded-lg flex items-center justify-center flex-shrink-0">
              ...
              <div>
                <p class="text-xs font-bold text-[#141414]">Friend plays games</p>
                <p class="text-[10px] text-neutral-500">1% BUX / 0.2% ROGUE from losing bets</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="pb-12">
        <div class="bg-white rounded-2xl border border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden">
          <div class="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
            <h3 class="text-sm font-bold text-[#141414]">Earnings History</h3>
            <span class="text-xs text-neutral-400 flex items-center gap-1.5 font-mono">
            ...
            <%= if @earnings == [] do %>
              <div class="flex flex-col items-center justify-center py-16 text-center">
                <div class="w-16 h-16 bg-neutral-100 rounded-2xl flex items-center justify-center mb-4">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-8 h-8 text-neutral-300">
                    <path d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                  </svg>
                </div>
                <p class="text-sm font-bold text-[#141414] mb-1">No earnings yet</p>
                <p class="text-xs text-neutral-400 max-w-[240px]">Share your referral link to start earning BUX and ROGUE.</p>
              </div>
            <% else %>
              <table class="w-full text-sm min-w-[500px]">
                <thead class="sticky top-0 bg-neutral-50 z-10">
                  <tr>
                    <th class="px-5 py-3 text-left text-[10px] uppercase tracking-[0.16em] font-bold text-neutral-500">Type</th>
                    <th class="px-5 py-3 text-left text-[10px] uppercase tracking-[0.16em] font-bold text-neutral-500">From</th>
                    <th class="px-5 py-3 text-right text-[10px] uppercase tracking-[0.16em] font-bold text-neutral-500">Amount</th>
                    <th class="px-5 py-3 text-left text-[10px] uppercase tracking-[0.16em] font-bold text-neutral-500">Time</th>
                    <th class="px-5 py-3 text-left text-[10px] uppercase tracking-[0.16em] font-bold text-neutral-500">TX</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-neutral-50">
                  <%= for earning <- @earnings do %>
                    <tr class={"hover:bg-neutral-50/50 transition-colors " <> if(recent?(earning.timestamp), do: "bg-[#CAFC00]/5", else: "")}>
                      <td class="px-5 py-3.5">
                        <span class={"inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold " <> type_style(earning.earning_type)}>
                          {type_label(earning.earning_type)}
                        </span>
                      </td>
                      <td class="px-5 py-3.5 text-xs font-mono text-neutral-400">
                        {truncate_wallet(earning.referee_wallet)}
                      </td>
                      <td class="px-5 py-3.5 text-right">
                        <span class="text-sm font-bold text-[#141414]">{format_amount(earning.amount)}</span>
                        <span class="text-[10px] text-neutral-400 ml-1">{earning.token}</span>
                      </td>
                      ...
                      <td class="px-5 py-3.5">
                        <%= if earning.tx_hash do %>
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            class="text-xs text-neutral-400 hover:text-[#141414] transition-colors cursor-pointer font-mono"
                          >
                            View ↗
                          </a>
                        <% else %>
              ...
              <%= unless @end_reached do %>
                <div class="flex justify-center py-4">
                  <div class="w-5 h-5 border-2 border-neutral-200 border-t-[#CAFC00] rounded-full animate-spin"></div>
                </div>
              <% end %>
            <% end %>
          </div>
