defmodule BlocksterV2Web.NotificationLive.Index do
  ...
  defp category_icon_bg("system"), do: "bg-neutral-100"
  defp category_icon_bg(_), do: "bg-neutral-100"
  ...
  defp category_icon("offers") do
    Phoenix.HTML.raw("""
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 text-[#8B8B00]">
    ...
    <main class="max-w-[720px] mx-auto px-6">
      ...
      <section class="pt-12 pb-6">
        <div class="flex items-center justify-between mb-4">
          <div>
            <div class="mb-3">
              <BlocksterV2Web.DesignSystem.eyebrow>Notifications</BlocksterV2Web.DesignSystem.eyebrow>
            </div>
          ...
          <div class="flex items-center gap-2">
            <.link
              navigate={~p"/notifications/referrals"}
              class="flex items-center justify-center w-10 h-10 rounded-xl bg-white border border-neutral-200/70 hover:border-neutral-300 hover:shadow-sm transition-all cursor-pointer"
              title="Referral Dashboard"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4.5 h-4.5 text-neutral-500">
      ...
      <%!-- ━━━ FILTER ROW ━━━ --%>
      <section class="py-4 border-t border-neutral-200/70">
        <div class="flex items-center justify-between flex-wrap gap-3">
          <div class="flex items-center gap-2">
            <%= for {value, label} <- [{"all", "All"}, {"unread", "Unread"}, {"read", "Read"}] do %>
              <button
                ...
                class={"inline-flex items-center px-3.5 py-1.5 rounded-full text-[11px] font-medium tracking-[0.04em] transition-colors cursor-pointer border " <>
                  if(@active_filter == value, do: "bg-[#0a0a0a] text-white border-[#0a0a0a]", else: "bg-white text-neutral-600 border-neutral-200/70 hover:border-neutral-300")}
              >
                {label}
              </button>
            <% end %>
          ...
          <div class="flex items-center gap-3">
            <%= if @unread_count > 0 do %>
              <span class="text-[10px] font-mono text-neutral-400 tabular-nums">
                {Integer.to_string(@unread_count)} unread
              </span>
            <% end %>
            <%= if Enum.any?(@notifications, &is_nil(&1.read_at)) do %>
              <button
                phx-click="mark_all_read"
                class="inline-flex items-center gap-1.5 text-[11px] font-medium text-neutral-500 hover:text-[#141414] cursor-pointer transition-colors"
              >
            ...
            <div class="flex flex-col items-center justify-center py-24 text-center">
              <div class="relative mb-6">
                <div class="w-20 h-20 bg-neutral-100 rounded-2xl flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-10 h-10 text-neutral-300">
                    <path fill-rule="evenodd" d="M5.25 9a6.75 6.75 0 0 1 13.5 0v.75c0 2.123.8 4.057 2.118 5.52a.75.75 0 0 1-.297 1.206c-1.544.57-3.16.99-4.831 1.243a3.75 3.75 0 1 1-7.48 0 24.585 24.585 0 0 1-4.831-1.244.75.75 0 0 1-.298-1.205A8.217 8.217 0 0 0 5.25 9.75V9Zm4.502 8.9a2.25 2.25 0 1 0 4.496 0 25.057 25.057 0 0 1-4.496 0Z" clip-rule="evenodd" />
                  </svg>
                </div>
              ...
              <p class="text-lg font-bold text-[#141414] mb-1">All caught up</p>
              <p class="text-sm text-neutral-400 max-w-[260px]">Nothing new right now. We'll notify you when something happens.</p>
            </div>
          <% else %>
            <%= for notification <- @notifications do %>
              <div
                id={"notification-#{notification.id}"}
                class={"relative flex items-start gap-3 p-4 rounded-2xl transition-all cursor-pointer group border " <>
                  if(is_nil(notification.read_at),
                    do: "bg-white border-neutral-200/70 shadow-[0_1px_3px_rgba(0,0,0,0.04)]",
                    else: "bg-white/50 border-neutral-100 hover:bg-white hover:border-neutral-200/70")}
              ...
              >
                ...
                <div class={"flex-shrink-0 " <> if(is_nil(notification.read_at), do: "ml-1", else: "")}>
                  <%= if notification.image_url do %>
                    ...
                    />
                  <% else %>
                    <div class={"w-11 h-11 rounded-xl flex items-center justify-center " <> if(is_nil(notification.read_at), do: category_icon_bg(notification.category), else: "bg-neutral-100")}>
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-start justify-between gap-2">
                    <p class={"text-sm leading-snug " <> if(is_nil(notification.read_at), do: "font-bold text-[#141414]", else: "text-neutral-500")}>
                      {notification.title}
                    </p>
                    <div class="flex items-center gap-2 flex-shrink-0">
                      <span class="text-[10px] whitespace-nowrap text-neutral-400 font-mono">{format_time(notification.inserted_at)}</span>
                      <%= if is_nil(notification.read_at) do %>
                        <div class="w-2 h-2 rounded-full bg-[#CAFC00] flex-shrink-0 shadow-[0_0_6px_rgba(202,252,0,0.4)]"></div>
                      <% end %>
                    </div>
                  </div>
                  <p class={"text-xs mt-0.5 line-clamp-2 " <> if(is_nil(notification.read_at), do: "text-neutral-500", else: "text-neutral-400")}>{notification.body}</p>
                  <%= if notification.action_label do %>
                    <p class={"text-xs mt-1.5 font-medium group-hover:underline " <> if(is_nil(notification.read_at), do: "text-[#141414]", else: "text-neutral-400")}>{notification.action_label}</p>
                  <% end %>
                </div>
                ...
                <%= if is_nil(notification.read_at) do %>
                  <button
                    phx-click="mark_read"
                    phx-value-id={notification.id}
                    class="flex-shrink-0 p-1.5 rounded-lg text-neutral-300 hover:text-[#141414] hover:bg-neutral-50 opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
                    title="Mark as read"
                  >
            ...
            <%= unless @end_reached do %>
              <div class="flex justify-center py-6">
                <div class="w-6 h-6 border-2 border-neutral-200 border-t-[#CAFC00] rounded-full animate-spin"></div>
              </div>
            <% end %>
          <% end %>
        </div>
