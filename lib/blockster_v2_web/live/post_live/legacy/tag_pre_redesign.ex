defmodule BlocksterV2Web.PostLive.Legacy.TagPreRedesign do
  @moduledoc """
  @deprecated Pre-redesign tag browse page. Preserved for reference.
  The active module is `BlocksterV2Web.PostLive.Tag`.
  """
  ...
  def mount(%{"tag" => tag_slug}, _session, socket) do
    ...
    case Blog.get_tag_by_slug(tag_slug) do
      ...
      tag ->
        ...
        {components, displayed_post_ids, bux_balances} = build_initial_components(tag.slug, "latest", 0)
  ...
  def handle_event("load-more", _, socket) do
    ...
    {new_components, new_displayed_post_ids, new_bux_balances} =
      build_components_batch(tag_slug, displayed_post_ids, last_module, sort_mode, inline_banner_offset)
  ...
  defp build_initial_components(tag_slug, sort_mode, inline_banner_offset) do
    build_components_batch(tag_slug, [], BlocksterV2Web.PostLive.PostsSixComponent, sort_mode, inline_banner_offset)
  end
  ...
  defp build_components_batch(tag_slug, displayed_post_ids, last_module, _sort_mode, inline_banner_offset) do
    ...
    {components, final_displayed_ids, bux_balances} =
      Enum.reduce(0..3, {[], displayed_post_ids, %{}}, fn idx, {acc_components, acc_ids, acc_balances} ->
        ...
        posts = Blog.list_published_posts_by_date_tag(
          tag_slug,
          limit: posts_needed,
          exclude_ids: acc_ids
        )
